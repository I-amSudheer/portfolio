...
export function Contact() {
  ...
  return (
    <Section id="contact" eyebrow="Contact" title="Let's Build Something" description="Reach out for freelance work, collaborations, or just to say hi.">
      <div className="grid lg:grid-cols-[1fr_1.3fr] gap-8">
          ...
          {socials.map((s, i) => (
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">{s.href.replace(/^https?:\/\//, "").replace("mailto:", "")}</div>
              </div>
            </motion.a>
          ))}
        </motion.div>
        ...
        >
          ...
          >
            {sent ? (<><CheckCircle2 className="w-5 h-5" /> Message Sent!</>) : (<>Send Message <Send className="w-4 h-4" /></>)}
          </motion.button>
        </motion.form>
      </div>
    </Section>
