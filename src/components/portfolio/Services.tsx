...
export function Services() {
  return (
    <Section id="services" eyebrow="Services" title="Freelance Web Development Services" description="Hire me to design, build, secure, and maintain your next product.">
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((s, i) => (
          ...
          >
            ...
            <div className="font-semibold text-lg mb-2 group-hover:text-primary-foreground transition-colors">{s.title}</div>
            <div className="text-sm text-muted-foreground group-hover:text-primary-foreground/90 transition-colors">{s.desc}</div>
          </motion.div>
        ))}
      </div>
    </Section>
